<%@ Page Title="" Language="C#" MasterPageFile="~/MainContent.Master" AutoEventWireup="true"
    CodeBehind="CheckUpCategorized.aspx.cs" Inherits="Chm.Web.Admin.Checkup.CheckUpCategorized" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script language="javascript" type="text/javascript">
        ...
        function showDiv() {
            var url = "CheckUpCategorizedAdd.aspx" + urlParam;
            OpenWindow(url);
            return false;
        }
        $(document).ready(function () {
            ...
            $("#<%=btnDel.ClientID %>").click(function () {
                ...
                if (checkIsSelected('<%=gvCheckUp.ClientID %>', pleaseSelect)) {
        ...
        function fn_LinkAddPage(did, patientId) {
            var url = "CheckUpCategorizedAdd.aspx?DID=" + did + "&PatientId=" + patientId;
            OpenWindow(url);
            return false;
        }
    </script>
            ...
            <td>
                <div class="con">
                    <ul id="tags"  class="clearfix mb_10">
                        <li class="selectTag" id="liCheckUp"><a>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" class="searchPanel" align="center">
                            <tr>
                                ...
                                <td class="add">
                                    ...
                                    <Chy:XGridView ID="gvCheckUp" runat="server" CssClass="zebratable" AllowPaging="true"
                                        AutoGenerateColumns="False" PagerSettings-PageButtonCount="8" PageSize="10" Width="100%"
                                        OnRowDataBound="gvCheckUp_RowDataBound">
                                        <Columns>
                                            <asp:TemplateField HeaderStyle-CssClass="check" ItemStyle-CssClass="check" HeaderStyle-Width="3%"
                                                ItemStyle-Width="3%">
                                                <HeaderTemplate>
                                                    <input id="chkSelAll" type="checkbox" onclick="selectAll(this,'<%=gvCheckUp.ClientID %>')" />
                                                </HeaderTemplate>
                                                <ItemTemplate>
                                                    <input name="chk" id="chkselect<%# Eval("CheckRptID") %>" type="checkbox" value="<%# Eval("DID") %>" />
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                        </Columns>
                                        <Columns>
                                            <asp:BoundField DataField="DiseaseName"></asp:BoundField>
                                            <asp:BoundField DataField="TimeInGroup" DataFormatString="{0:yyyy-MM-dd}"></asp:BoundField>
                                            <asp:BoundField DataField="RDate" DataFormatString="{0:yyyy-MM-dd}"></asp:BoundField>
                                            <asp:TemplateField>
                                                <ItemTemplate>
                                                    <asp:Label ID="lblReportId" Style="display: none" runat="server" Text='<%#Eval("CheckRptID").ToString()%>'></asp:Label>
                                                    &nbsp;&nbsp;<%#GetDataSourceType(Eval("CheckRptID").ToString())%>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                            <asp:TemplateField Visible="false">
                                                <ItemTemplate>
                                                    &nbsp;&nbsp;<asp:LinkButton ID="lbtnEdit" runat="server"></asp:LinkButton>
                                                    <asp:Label ID="lbtnResult" Visible="false" Text='<%# Eval("DID") %>' runat="server"></asp:Label>
                                                </ItemTemplate>
                                            </asp:TemplateField>
                                        </Columns>
                                    </Chy:XGridView>
